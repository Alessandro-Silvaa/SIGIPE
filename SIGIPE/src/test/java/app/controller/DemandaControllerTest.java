...
public class DemandaControllerTest {
	...
	void setup() {
		...
		when(this.demandaRepository.save(new Demanda())).thenReturn(new Demanda());
	...
	@Test
	void update200() {
		ResponseEntity<Demanda> response =  this.demandaController.update(new Demanda(), 1);
		int httpStatus = response.getStatusCode().value();
		assertEquals(200, httpStatus);
	}
	...
	@Test
	void save200() {
		ResponseEntity<Demanda> response =  this.demandaController.save(new Demanda());
		int httpStatus = response.getStatusCode().value();
		assertEquals(200, httpStatus);
